...
function AssetForm({
  ...
  const onAssetTypeSelected = (assetType) => {
    ...
    setAssetDetails((prevDetails) => ({
      ...prevDetails,
      asset_class: Object.keys(assetClassToType).find(
        (key) => assetClassToType[key] === assetType
      ),
    }));
  };
            ...
            <SupportedTypeSelector
              items={
                editMode
                  ? assetTypes
                  : assetTypes.filter(
                      (item) => item !== "DEFAULT" && item !== "NO_CONTEXT"
                    )
              }
          ...
          <Box display="flex" flexDirection="row" gap={2}>
            ...
            {editMode ? (
              <>
                ...
                {assetDetails.asset_class !== "default" ? (
                 !isEditing &&  <Button
                    sx={{ fontWeight: 400 ,width:'90px'}}
                    color="error"
                    variant="outlined"
              ...
              assetType && (
                <Button
                  color="success"
                  sx={{ fontWeight: 400,minWidth:'90px' }}
                  variant="outlined"
