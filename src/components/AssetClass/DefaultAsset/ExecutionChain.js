...
function ExecutionChain({ asset, handleInputChange, isEditing, bot }) {
  ...
  const onDelete = (item) => {
    ...
    const new_chain = asset.execution_chain.filter(
  ...
  return (
    <div>
      <div className="flex gap-6 flex-col ">
        {supportedStrategies ? (
          <SupportedTypeSelector
            ...
            currentItem={
              asset.execution_strategy
                ? asset.execution_strategy
                : supportedStrategies[0]
            }
            onItemSelected={(value) =>
              handleInputChange(value, "execution_strategy")
            }
          ...
          <Typography color={"red"}>
            {" "}
            Failed getting supporting strategies{" "}
          </Typography>
        )}
     ...
     {asset && asset.execution_chain && (
        <ChipWithArrow
          selectedItems={asset.execution_chain.map((item) => ({
            key: item,
            label: item,
          }))}
          onDelete={onDelete}
